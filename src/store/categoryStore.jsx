...
import { apiAddCategory, apiDeleteCategory, apiGetAllMorph, apiGetAllSnake, apiGetCategory, apiGetMorphByCategory, apiUpdateCategory } from "../api/ApiCategory";
    ...
    (set, get) => ({
      ...
      getAllSnake: async () => {
        try {
          ...
          set({ CategoryMorphList: {selectCategory:res.data.AllSnake} })
        } catch (err) {
          console.log(err)
          // toast.error(err.response.data.message);
        }
      ...
      getMorphByCategory: async (morphId,categoryId) => {
        try {
          ...
          set({ CategoryMorphList: res.data })
