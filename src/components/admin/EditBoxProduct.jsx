...
const CreateBoxProduct = ({
  ...
  useEffect(() => {
    // เมื่อมีการเปลี่ยนแปลง categoryId ให้ตั้งค่า morph ใหม่  เพราะตอนแรก ชื่อmorph ไม่ยอมขึ้นใน input
    const selectedMorphs = CategoryAllMorph.filter(
      (morph) => morph.categoryId == form.categoryId
    );
  ...
  const hdlEditSnake = async () => {
    ...
    await UpdateSnake(editCategoryId.id, formData, token);
    await getAllSnake();
    setModal(!modal);
  };
